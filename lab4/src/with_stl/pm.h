...
namespace PackageManager 
{
	...
	public:
		...
		virtual std::list<MainPackage*> unite_packages(std::ostream&, const std::list<MainPackage*>&); // ???????
	...
	public:
		...
		std::list<MainPackage*> separate(unsigned, const std::pair<char*, 
...
//			int *f;
//			int time;
//			DFSParameters(int size) 
//				: color(new colors[size])
//				, pred(new MainPackage*[size])
//				, d(new int[size])
//				, f(new int[size])
//				, time(0) {
...
//				}
//			}
//			~DFSParameters() noexcept
//				{ delete [] color; delete [] pred; delete [] d; delete [] f; }
//		};
//
//		void dfs_visit(DFSParameters&, int) const noexcept;
	...
	public:
		...
		void remove_nonused_auxiliary_packages() noexcept;
	};
