...
namespace SparseMatrix
{
	...
	int *get_index(const char *msg, int &n)
	{
		...
		do {
			...
			std::cout << msg;
			err_msg = "Index shoud be a natural number...";
			if (get_num(n) < 0) {
				return nullptr;
			} 
		}	while (n < 1);
	...
	void sort_row(Row *&row, int n, int n_nonzero)
	{
		if (!row || !row->first) {
			return;
		}
		...
		memset(temp_arr + n_nonzero, 0, sizeof(double) * (n - n_nonzero));
	...
	void change_matrix(Matrix *&arr)
	{
		...
		sort_row(max_sum_row, arr->n_col, max_sum_row->not_zero);
	}
